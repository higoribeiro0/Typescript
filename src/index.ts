...
function findSpaceship(name: string) {
    ...
    spaceship = spaceships.find((ship) => { //Enviar
...
function sendInMission(spaceship: { name: string, crewLimit: number, crew: string[], inMission: boolean }) {
    ...
    } else if (spaceship.crew.length < Math.floor(spaceship.crewLimit / 3)) { //floor: arrendondar para menor número.
...
function fourthMenuOption() {
    ...
    spaceships.forEach((spaceship: {  //executar cada item do array.
        name: string,
        pilot: string,
        crewLimit: number,
        crew: string[],
...
while (userOption !== 5) {
    ...
    userOption = Number.parseInt(prompt(menu))//Converter uma string para inteiro.
